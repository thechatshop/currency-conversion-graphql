...
const fetchRates = async () => {
	const res = await fetch(`https://openexchangerates.org/api/latest.json?app_id=${process.env.API_KEY}`)
	return res.json();
}
...
const buildRates = (rates, currencies) => {
	if (!isNil(currencies) && !isEmpty(currencies)) {
		return currencies.map(currency => (rates.find(rate => rate.abbreviation === currency)));
	}
	return rates;
}
